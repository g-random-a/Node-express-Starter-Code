...
import { getChannel, QUEUE_NAME } from '../../config/rabbitMq'; // Updated path
  ...
  try {
    ...
    await channel.consume(
      ...
      (msg) => {
        if (msg) {
          console.log(
            `Received message from queue "${QUEUE_NAME}": ${msg.content.toString()}`,
          );
    ...
    if (retries < MAX_RETRIES) {
      retries++;
      console.error(
        `RabbitMQ consumer error. Retrying in ${
          RETRY_DELAY_MS / 1000
        } seconds...`,
      );
